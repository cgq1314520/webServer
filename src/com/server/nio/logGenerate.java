...
public class logGenerate {
     ...
     * @param client  连接通道
     */
    protected static void logger(LoggerType type, String content, SocketChannel client){
        //用来存储连接后的日志信息
        StringBuilder logInf=new StringBuilder();
        try {
            switch (type) {
                case FORBIDDEN:
                    client.write(ByteBuffer.wrap((type.getHeader()+type.getContent()+type.getEnd()).getBytes()));
                    client.close();
                ...
                case ERROR_REQUEST:
                    client.write(ByteBuffer.wrap((type.getHeader()+type.getContent()+type.getEnd()).getBytes()));
                    client.close();
                    logInf.append("Error reqeust").append(content);
                    break;
                case SYSTEM_EXCEPTION:
                    client.write(ByteBuffer.wrap((type.getHeader()+content+type.getEnd()).getBytes()));
                    client.close();
                    logInf.append("System exception").append(content);
                    break;
                case LOG:
    ...
    protected static void logger(LoggerType type, String content, Socket client){
        ...
        try {
            switch (type) {
                case FORBIDDEN:
                    client.getOutputStream().write((type.getHeader()+type.getContent()+type.getEnd()).getBytes());
                    logInf.append("Forbidden").append(content);
                    break;
                case NOTFOUND:
                    client.getOutputStream().write((type.getHeader()+type.getContent()+type.getEnd()).getBytes());
